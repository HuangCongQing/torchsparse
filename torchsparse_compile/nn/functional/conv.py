...
def conv3dwbn(
    ...
    if (kernel_size == 1 and stride == 1 and dilation == 1):
        ...
        if input_kmap is None:
            ...
            offsets = get_kernel_offsets(
                kernel_size, # 3
                stride=input_stride, # 1
                dilation=dilation, # 1
                device=input_feat.device,
            )

            references = F.sphash(input_coord) # (84000,)
            queries = F.sphash(output_coord.int(), offsets) # (27, 84000)如何偏移量????
            input_kmap = F.sphashquery(queries, references) # (27, 84000) input_kmap()<<<<<<<<<<<<<<<<<<<<<

        nbsizes = torch.sum(input_kmap != -1, dim=1) # (27, ) 某个位置对应有点数量
        nbmaps = torch.nonzero(input_kmap != -1) # (328938, 2) https://www.yuque.com/huangzhongqing/pytorch/gck7a0#UptnH
        indices = nbmaps[:, 0] * input_kmap.size(1) + nbmaps[:, 1] # x* 84000 + y二维坐标转化为1维形式坐标
        # nbmaps[:, 1] 是84000的index
        nbmaps[:, 0] = input_kmap.view(-1)[indices] # (328938, 2) 选出这些1维形式下标变为0维横坐标
        nbmaps = nbmaps.int()
        nbsizes = nbsizes.int()
        sizes = torch.Tensor([input_coord.shape[0], output_coord.shape[0]]).int() #[84000, 84000]
        output_feat = SparseConvolutionFunction(
            input_feat,
            weight,
            nbmaps, # <<<<<<<<<<<<<<<<<<,,,,
